...
class Minimizer(object):
    ...
    def state_dict(self):
        ...
        """
        return {key: value for key, value in self.__dict__.items() if key not in ['model', 'optimizer', 'state']}
...
class GSAM(SAM):
    def __init__(self, optimizer, model, rho, alpha=0.1,
...
class BridgedNSAM(SAM):
    ...
    def middle_step(self):
        ...
        for n, p in self.model.named_parameters():
            if p.grad is None:
                continue
            self.state[p]["grad"] = self.descents_weights[self.n] * p.grad.clone().detach()  # Add grad
            grads.append(torch.norm(p.grad.clone().detach(), p=2))  # Current grad to calculate norm
        grad_norm = torch.norm(torch.stack(grads), p=2) + 1.e-16
...
class AWP(SAM):
    ...
    def ascent_step(self):
        ...
        for n, p in self.model.named_parameters():
            ...
            grads.append(torch.norm(p.grad, p=2))
        weight_norm = torch.norm(torch.stack(weights), p=2) + 1.e-16
        grad_norm = torch.norm(torch.stack(grads), p=2) + 1.e-16
