...
class AdvTrainer(Trainer):
    ...
    def record_rob(self, train_loader, val_loader,
        ...
        self.rob_dict['loaders'] = {'(Tr)':self.rob_dict['train_loader'],
    ...
    def record_during_eval(self):
        for flag, loader in self.rob_dict['loaders'].items():
            ...
            if eps is not None:
                self.dict_record['FGSM'+flag] =\
                    self.rmodel.eval_rob_accuracy_fgsm(loader,
                                                       eps=eps,
                                                       verbose=False)
                steps = self.rob_dict.get('steps')
                alpha = self.rob_dict.get('alpha')
                if steps is not None:
                    self.dict_record['PGD'+flag] =\
                        self.rmodel.eval_rob_accuracy_pgd(loader,
                                                          eps=eps,
                                                          alpha=alpha,
                                                          steps=steps,
                                                          verbose=False)
            ...
            if std is not None:
                self.dict_record['GN'+flag] =\
                    self.rmodel.eval_rob_accuracy_gn(loader,
                                                     std=std,
                                                     verbose=False)
