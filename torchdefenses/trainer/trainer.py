...
class Trainer():
    ...
    def setup(self, optimizer,
              ...
              n_epochs=None, n_iters=None,
              minimizer=None, clip_grad_norm=None):
        params = self.rmodel.parameters()
        self.optimizer = self.generate_optimizer(optimizer, params)
        self.scheduler, self.scheduler_type =\
            self.generate_schdeuler(scheduler, self.optimizer, scheduler_type, n_epochs, n_iters)
    ...
    def load(path):
        pass
        # dict_save = torch.load(save_path, map_location='cpu')

        # keys_with_dict = ["rmodel", "optimizer", "scheduler", "minimizer"]
        # keys_without_dict = ["curr_epoch", "curr_iter", "scheduler_type", "recordmanager"]
        # for key in dict_save.keys():
        #     if key in keys_with_dict:
        #         exec("self."+key+".load_state_dict(dict_save[key])")
        #     if key in keys_without_dict:
        #         exec("self."+key+"= dict_save[key]")

        # print("Trainer loaded.")
        # if 'record_info' in dict_save.keys():
        #     print("Record Info:")
        #     print(dict_save['record_info'])

    def fit(self, train_loader, n_epochs, n_iters=None,
            record_type="Epoch", save_path=None,
            save_type="Epoch", save_best=None,
        ...
        self.rm.initialize(record_type=record_type,
                           save_path=save_path,
                           best_option=save_best)
        ...
        for epoch in range(n_epochs):
            ...
            for i, train_data in enumerate(train_loader):
                ...
                if save_path is not None:
                    ...
                    is_save_condition = self._check_run_condition(save_type, is_last_batch)
    ...
    def generate_optimizer(optimizer, params):
        if isinstance(optimizer, str):
            optimizer = eval(optimizer.split("(")[0] + "(params," + optimizer.split("(")[1])
        return optimizer

    @staticmethod
    def generate_schdeuler(scheduler, optimizer, scheduler_type=None, n_epochs=None, n_iters=None):
        if isinstance(scheduler, str):
            ...
            # Cyclic(base_lr=0, max_lr=0.3)
                ...
                scheduler = eval("CyclicLR(optimizer, " + scheduler.split("(")[1].split(")")[0] + \
            ...
            # Cosine
                ...
                scheduler = eval(scheduler.split("(")[0] + "(optimizer, " + scheduler.split("(")[1])
    ...
    def generate_minimizer(minimizer, model, optimizer):
        if isinstance(minimizer, str):
            minimizer = eval(minimizer.split("(")[0] + "(model, optimizer," + minimizer.split("(")[1])
        return minimizer
    ...
    def _check_valid_save_path(self, save_path, save_type, save_overwrite):
        if save_path is not None:
            ...
            self._check_and_create_path(save_path+"/init.pth", overwrite=save_overwrite, file=True)
            self._check_and_create_path(save_path+"/last.pth", overwrite=save_overwrite, file=True)
            self._check_and_create_path(save_path+"/best.pth", overwrite=save_overwrite, file=True)
            self._check_and_create_path(save_path+"/log.txt", overwrite=save_overwrite, file=True)

            if save_type in ["Epoch", "Iter"]:
                self._check_and_create_path(save_path+"/epoch_iter/", overwrite=save_overwrite)
                self.save_dict(save_path, 0)
        else:
            if save_type is not None:
                raise ValueError("save_path should be given for save_type != None.")

    def save_dict(self, save_path, save_type=None,
                  is_init=False, is_best=False):
        ...
        if save_type == "Epoch":
            save_name = "/epoch_iter/%s_%s.pth"%(str(self.accumulated_epoch).zfill(5),
                                                 str(0).zfill(5))
    ...
    def load_dict(self, save_path):
        for key in ['accumulated_epoch', 'accumulated_iter', 'scheduler_type']:
            setattr(trainer, key, d[key])

        for key in ['rmodel', 'optimizer', 'scheduler', 'minimizer']:
            value = getattr(trainer, key)
            if value is not None:
                value.load_state_dict(d[key])
