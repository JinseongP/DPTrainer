...
class RecordManager(object):
    ...
    def __repr__(self):
        return "%s(keys=[%s])"%(self.__class__.__name__, ", ".join(self.records.keys()))

    def initialize(self, record_type=None,
                   save_path=None,
                   best_option=None):
    ...
    def progress_end(self):
        ...
        self.print_only("Progress: " + next(self._spinner) + " [" + str(t)+"/it]" + " "*20, end='\r')
    ...
    def _print_head(self, dict_print, slack=3):
        ...
        self._print_form = "".join(['{:<'+str(length)+'.'+str(length)+'}' for length in lengths])
    ...
    def plot(self, inputs, x=None, figsize=(6,6), title="", xlabel="", ylabel="",
        ...
        if ver == 1:
            for input in inputs:
                plot_line(ax, input, x, linestyle=linestyles[i], linewidth=linewidths[i],
            ...
            for j, input in enumerate(inputs):
                plot_line(axes[js[j]], input, x, linestyle=linestyles[i], linewidth=linewidths[i],
                          color=colors[i], label=labels[i], alpha=alphas[i])
                i += 1
