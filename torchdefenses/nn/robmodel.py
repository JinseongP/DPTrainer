...
class RobModel(nn.Module):
    ...
    def __init__(self, model, n_classes,
                 normalization_used={'mean': [0., 0., 0.], 'std': [1., 1., 1.]},
                 device=None):
        super().__init__()
    ...
    def add_normalize(self):
        ...
        self.model = nn.Sequential(
            Normalize(self.mean.cpu().numpy(), self.std.cpu().numpy()),
            self.model
        )
    ...
    def eval_rob_accuracy_pgd(self, data_loader, eps, alpha, steps,
                              random_start=True, restart_num=1, norm='Linf',
                              **kargs):
        if norm == 'Linf':
    ...
    def eval_rob_accuracy_autoattack(self, data_loader, eps, version='standard', norm='Linf', 
                                     **kargs):
