...
class Trainer():
    ...
    # Init Minimizer
    def _init_minimizer(self, minimizer, augmult):
    ...
    def _check_record_best(self, save_best, early_stopping=True):
        if self.best_record_dict is not None:
            ...
            for key in self._HBS:
                ...
                if best_value > curr_value:
                    # if early_stopping:
                    #     if curr_value < best_value//2:
                    #         print (f'Early stopping!! Best value: {best_value}, Curr Value: {curr_value}') 
                    #         sys.exit(1)                    
                    return False
